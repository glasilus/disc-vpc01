...
EFFECTS: List[EffectSpec] = [
    ...
    EffectSpec(
        ...
        tooltip='Darkens every Nth row. Higher INTENSITY = darker and denser lines.',
    ),
    EffectSpec(
        ...
        tooltip='Drops the lowest bits per channel. Maps 8-bit colour into 1-7 levels — extreme posterise look.',
    ),
    EffectSpec(
        ...
        tooltip='Re-encodes the frame as JPEG at quality 1-40 (intensity-driven) and decodes it back. Block-edge artefacts everywhere.',
    ),
    EffectSpec(
        ...
        tooltip='Bulges the image outward from the centre. Use low CHANCE — it is visually heavy.',
    ),
    EffectSpec(
        ...
        tooltip='Shifts horizontal strips by simplex-noise amounts and adds per-pixel luminance noise. Authentic VHS scan glitch.',
    ),
    EffectSpec(
        ...
        tooltip='Even rows = current frame, odd rows = previous frame. Authentic 50i look on motion.',
    ),
    EffectSpec(
        ...
        tooltip='Sprays random-coloured vertical bars and rolls random rows horizontally. Digital signal breakup.',
    ),
    EffectSpec(
        ...
        tooltip='Quantises to 2-16 levels per channel through a 4×4 Bayer matrix. Pixel-art / GameBoy palette feel.',
    ),
    EffectSpec(
        ...
        tooltip='Crops the centre and upscales it back with INTER_NEAREST. Punchy on hits.',
    ),
    ...
    EffectSpec(
        ...
        tooltip='Accumulator = current·(1−w) + accumulator·w. IMPACT clears it. Builds wash-style trails on sustained energy.',
    ),
    EffectSpec(
        ...
        tooltip='Splits the frame into horizontal bands; even bands roll left, odd bands roll right by INTENSITY·width.',
    ),
    EffectSpec(
        ...
        tooltip='Down-up resampling produces blocky pixelation. Block size scales with intensity (4-44 px).',
    ),
    EffectSpec(
        ...
        tooltip='Blends current·0.5 + frame_N_ago·0.3 + frame_2N_ago_hue+30°·0.2. Triple-exposure feel with a colour shift.',
    ),
    EffectSpec(
        ...
        tooltip='hstack(frame, frame[:,::-1]) → vstack(_, 255-_) → rotate by intensity·180°. Symmetry mandala.',
    ),
    EffectSpec(
        ...
        tooltip='Picks N random effects from {RGB, Block, Drift, Bitcrush}, N=intensity·4, applies them in sequence.',
    ),
    ...
    EffectSpec(
        ...
        params=[
            ParamSpec('fx_resonant_freq', 'Resonance Freq (cycles/px)', 0.08, 0.01, 0.3,
                      kwarg='cutoff',
                      tooltip='Centre frequency of the IIR bandpass. Lower = wider rings; higher = tight micro-detail ringing.'),
            ParamSpec('fx_resonant_q', 'Q factor (sharpness)', 12.0, 2.0, 30.0,
                      kwarg='q',
                      tooltip='Bandpass sharpness. >15 produces clearly audible-looking resonance bands at edges.'),
        ],
        note='IIR bandpass along pixel rows — spatial ringing at edges.',
        tooltip='Treats each row as audio and runs a 2nd-order bandpass filter. The output adds ringing to luminance edges.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_temporal_rgb_lag', 'Lag (frames)', 8, 2, 20, kind='int',
                          ...
                          tooltip='Max frames of separation between R, G and B. >10 frames creates obvious chroma ghost trails.')],
        note='R/G/B from different time offsets — chromatic time ghost.',
        tooltip='Reads each colour channel from a different past frame. Static scenes are unaffected; motion gets a rainbow trail.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_fft_phase_amount', 'Phase Noise Amount', 0.5, 0.05, 1.0,
                          kwarg='amount',
                          tooltip='Adds noise to FFT phase but keeps magnitude. Image scrambles into wave-interference patterns; >0.7 fully ungrounds it.')],
        note='Scrambles 2-D FFT phase, preserves magnitude — wave interference.',
        tooltip='Forward FFT, randomly shift phase, inverse FFT. Looks like a hologram corrupted in transit.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_waveshaper_drive', 'Drive', 3.0, 0.5, 8.0,
                          kwarg='drive',
                          tooltip='tanh saturation amount. 1=neutral, 3=warm, >5=hard clip / cartoon colours.')],
        note='Tube-amplifier saturation on pixel values.',
        tooltip='Maps pixels through tanh(drive · pixel)/tanh(drive). Soft-clip colour distortion that retains shape but punches saturation.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_histo_lag_frames', 'Palette Lag (frames)', 30, 5, 90, kind='int',
                          ...
                          tooltip='Match the histogram of the current frame to a frame N frames in the past. Big values = the colour palette feels stuck in time.')],
        note='Match palette to a frame from N back — colour memory.',
        tooltip='Histogram-matching against a delayed buffer. The composition stays current, the palette is from the past.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_wrong_sub_factor', 'Downsample Factor', 4, 2, 8, kind='int',
                          kwarg='factor',
                          tooltip='How aggressively to downsample chroma vs luma. 2 = mild, 8 = colour blocks visibly bleed past edges.')],
        note='Chroma subsampling abuse — colour blocks bleed over sharp edges.',
        tooltip='Downsamples Cr and Cb planes by FACTOR via INTER_AREA, upsamples back by INTER_NEAREST. Luma stays sharp.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_gameoflife_iters', 'Iterations', 2, 1, 5, kind='int',
                          kwarg='iterations',
                          tooltip='How many Conway steps to evolve the binarised mask. 1 = subtle, 5 = mask becomes alien.')],
        note='Conway automaton on frame as corruption mask — organic glitch.',
        tooltip='Binarise (>128), evolve N steps of Game of Life, XOR random noise into living cells. Bio-glitch overlay.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_ela_blend', 'ELA Blend (0=pure ELA, 1=original)', 0.5, 0.0, 1.0,
                          kwarg='blend',
                          tooltip='0 = full forensic heat-map, 1 = fully off. ~0.4 gives glow on edges over the original.')],
        note='JPEG compression error map — forensic edge glow.',
        tooltip='Re-compress frame at quality 75, take |diff|·amplify. Edges and high-frequency areas glow.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_dtype_corrupt_amount', 'Noise Amount', 0.05, 0.01, 0.4,
                          kwarg='amount',
                          tooltip='How hard to perturb the float16 view of the bytes. 0.05 = clean VRAM-glitch; 0.3 = total visual death.')],
        note='Frame bytes reread as float16 — VRAM-corruption look.',
        tooltip='Reinterprets the byte buffer as float16, adds Gaussian noise, views back as uint8. Looks like a corrupted GPU framebuffer.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_spatial_reverb_decay', 'Reverb Decay', 0.15, 0.05, 0.45,
                          kwarg='decay',
                          tooltip='Reflection strength. Higher = more pronounced echo trails along each row.')],
        note='Decaying horizontal echo — acoustic reverb on light.',
        tooltip='FFT-convolves each row with a sparse impulse response (6 reflections, decay-shaped). Light echoes laterally.',
    ),
    ...
    EffectSpec(
        ...
        params=[ParamSpec('fx_deriv_warp_blend', 'Prev Blend', 0.35, 0.0, 0.6,
                          kwarg='blend',
                          tooltip='How much of the previous frame ghosts through. 0 = pure displacement; 0.6 = heavy smear.')],
        note='IMPACT / NOISE / DROP / SUSTAIN — Sobel of prev frame as motion field.',
        tooltip='Closest CPU analogue to datamosh. Computes a gradient of the previous frame and uses it as an optical-flow-like vector field to displace the current frame.',
    ),
    EffectSpec(
        ...
        tooltip='Rotates pixels around the centre with angle = intensity · gaussian-falloff(radius). Subtle swirl at low intensity, full collapse at high.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_fractal_warp_octaves', 'Octaves', 4, 2, 5, kind='int',
                          kwarg='octaves',
                          tooltip='Number of noise scales summed. 2 = smooth blobs, 5 = jagged fractal.')],
        note='Any segment — fBm noise displacement field.',
        tooltip='Builds a multi-octave noise field and uses it as XY displacement. Field is reseeded per audio segment so it constantly evolves.',
    ),
    EffectSpec(
        ...
        params=[ParamSpec('fx_self_displace_depth', 'Depth (frames back)', 2, 1, 8, kind='int',
                          kwarg='depth',
                          tooltip='How many frames back the displacement source is taken from. Larger = more pronounced lag-induced tearing.')],
        note='IMPACT / NOISE / DROP / BUILD / SUSTAIN — past frame is the warp map.',
        tooltip='Past frame’s R channel = X offset, G channel = Y offset. Image literally uses its own colour to tear itself apart. With FEEDBACK active, cascades into datamosh-grade smear.',
    ),
    ...
    EffectSpec(
        ...
        params=[
            ParamSpec('fx_formula_expr', 'Expression', 'frame', kind='string', indent=False,
                      tooltip='NumPy expression. Available: frame, x, y, t, i, r, g, b, a, b, c, d, np, cv2, sin, cos, abs, clip. Returns uint8 HxWx3.'),
            ParamSpec('fx_formula_blend', 'Blend with original', 0.0, 0.0, 1.0,
                      kwarg='blend',
                      tooltip='0 = pure formula output, 1 = original frame. In-between cross-fades.'),
            ParamSpec('fx_formula_a', 'a', 0.5, 0.0, 1.0,
                      tooltip='Live slider — referenced as `a` inside the formula.'),
            ParamSpec('fx_formula_b', 'b', 0.5, 0.0, 1.0,
                      tooltip='Live slider — referenced as `b` inside the formula.'),
            ParamSpec('fx_formula_c', 'c', 0.5, 0.0, 1.0,
                      tooltip='Live slider — referenced as `c` inside the formula.'),
            ParamSpec('fx_formula_d', 'd', 0.5, 0.0, 1.0,
                      tooltip='Live slider — referenced as `d` inside the formula.'),
        ],
        extra_factory=_formula_extras,
        note='User-defined math expression evaluated per frame.',
        tooltip='Type any NumPy expression that produces an HxWx3 uint8 frame. Sandboxed: only numpy + safe builtins are exposed.',
    ),
    ...
    EffectSpec(
        ...
        params=[
            ParamSpec('fx_overlay_opacity', 'Opacity', 0.85, 0.0, 1.0, indent=False,
                      tooltip='Final alpha multiplier. 0.85 looks like a translucent decal.'),
            ParamSpec('fx_overlay_scale', 'Scale Max', 0.4, 0.05, 1.0, indent=False,
                      tooltip='Maximum size as a fraction of frame height. Intensity interpolates between min and max.'),
            ParamSpec('fx_overlay_scale_min', 'Scale Min', 0.15, 0.05, 1.0, indent=False,
                      tooltip='Minimum overlay size at intensity 0.'),
            ParamSpec('fx_overlay_blend', 'Blend Mode', 'screen', kind='choice',
                      choices=['screen', 'normal', 'multiply'], indent=False,
                      tooltip='screen brightens; multiply darkens; normal replaces.'),
            ParamSpec('fx_overlay_position', 'Position', 'random', kind='choice',
                      choices=['random', 'center', 'random_corner'], indent=False,
                      tooltip='Where on the frame to place the overlay. Decided once per segment.'),
            ParamSpec('fx_overlay_ck_mode', 'Chroma Key Mode', 'none', kind='choice',
                      choices=['none', 'dominant', 'secondary', 'manual'], indent=False,
                      tooltip='dominant = auto-key the most common hue; secondary = the second most common; manual = use the RGB below.'),
            ParamSpec('fx_overlay_ck_tolerance', 'CK Tolerance', 30, 5, 60, kind='int',
                      tooltip='How wide the keyed hue range is. Higher = more pixels removed.'),
            ParamSpec('fx_overlay_ck_softness', 'CK Edge Softness', 5, 1, 21, kind='int',
                      tooltip='Gaussian blur applied to the key mask. Higher = softer edges.'),
            ParamSpec('fx_overlay_ck_r', 'Manual Key R', 0, 0, 255, kind='int',
                      tooltip='Manual key colour red component.'),
            ParamSpec('fx_overlay_ck_g', 'Manual Key G', 255, 0, 255, kind='int',
                      tooltip='Manual key colour green component.'),
            ParamSpec('fx_overlay_ck_b', 'Manual Key B', 0, 0, 255, kind='int',
                      tooltip='Manual key colour blue component.'),
        ],
        extra_factory=_overlay_extras,
        note='Composites image/video files from the selected folder onto frames.',
        tooltip='Loads every PNG/JPG/MP4 in the selected folder and chooses one per active segment. Per-segment scale/position/chroma-key are decided once and held for the segment duration.',
    ),
]
